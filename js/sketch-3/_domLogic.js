function updateDOM() {
  if (getFoundTargets().length >= 12 + 6) {
    $('#instruction').html(`
    Awesome! You have found <span id="virtual-img-count">${
      getFoundTargets().length - 12
    }</span> new <span class="text-red-300 font-bold">reflected images</span>. 
    `);

    // show the following text
    $('#following-text').removeClass('hidden opacity-0');
    $('#following-text').addClass('');
  } else if (getFoundTargets().length > 12) {
    $('#instruction').html(`
    You have found <span id="virtual-img-count">${
      getFoundTargets().length - 12
    }</span> new <span class="text-red-300 font-bold">reflected images</span>, ${
      6 - (getFoundTargets().length - 12)
    } more to go.
    `);
