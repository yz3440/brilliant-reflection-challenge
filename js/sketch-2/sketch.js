...
function setup() {
  let myCanvas = createCanvas(windowWidth, 280);
...
function initBoundaries() {
  ...
  // top wall
  boundaries.push(
    new Boundary(marginX, marginY, width - marginX, marginY, false)
  );
  // bottom wall
  boundaries.push(
    new Boundary(
...
function initTargets() {
  ...
  for (let i = 1; i < 10; i++) {
    if (i === 2) {
      let preFound = new Target(width / 2 - i * boxW, marginY + 35, 8, true);
      preFound.hidden = false;
      targets.push(preFound);
