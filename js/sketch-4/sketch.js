...
const [boxW, boxH] = [90 * 1.5, 150 * 1.5];
...
function setup() {
  let myCanvas = createCanvas(windowWidth, 800);
  ...
  initBoundaries();
  initTargets();
  ray = new MouseControlledRay(width / 2, height / 2, translation);
...
function draw() {
  ...
  else if (ray.hasHitAnyTarget && ray.trace.length > 2) cursor('zoom-in');
  else cursor(ARROW);
  updateDOM();
}
...
function initTargets() {
  ...
  // y downward mirrored thrice
  targets.push(new Target(width / 2, marginY - 35 + 3 * boxH, 8, true));
  for (let i = 1; i < 10; i++) {
    targets.push(
      new Target(width / 2 + i * boxW, marginY - 35 + 3 * boxH, 8, true)
    );
    targets.push(
      new Target(width / 2 - i * boxW, marginY - 35 + 3 * boxH, 8, true)
    );
  ...
  // y downward mirrored four times
  targets.push(new Target(width / 2, marginY + 35 + 3 * boxH, 8, true));
  for (let i = 1; i < 10; i++) {
    targets.push(
      new Target(width / 2 + i * boxW, marginY + 35 + 3 * boxH, 8, true)
  ...
  // y downward mirrored five times
  targets.push(new Target(width / 2, marginY - 35 + 4 * boxH, 8, true));
  for (let i = 1; i < 10; i++) {
    targets.push(
      new Target(width / 2 + i * boxW, marginY - 35 + 4 * boxH, 8, true)
    );
    targets.push(
      new Target(width / 2 - i * boxW, marginY - 35 + 4 * boxH, 8, true)
    );
